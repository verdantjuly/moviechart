...
//upperSearch와 일치하는 영화를 담는 배열, filter를 이용해 필터링 한다.
let filteredtitlearray;
...
//for문 안에 선언해 주면 계속 초기화 되므로 바깥으로 뺀다.e
let populararray = [];
...
function load() {
    ...
    fetch('https://api.themoviedb.org/3/movie/top_rated?language=en-US&page=1', options)
        ...
        .then(data => {
            ...
            // movies[0]이 존재하지 않으면, 즉 movies라는 것이 빈 배열이라면
            if (!movies[0]) {
            ...
            for (i = 0; i < movies.length; i++) {
                ...
                popular = populararray.map(function (item) {
            ...
            movies.forEach((a) => {
                ...
                // 기본 주소값을 String으로 주고 w500은 weight, 시도 결과 일정 크기 미만은 엑박 오류가 난다.) 
                // 뒤에 경로를 API에서 찾아서 붙여 준다.
                poster_path = "https://image.tmdb.org/t/p/w500" + a['poster_path']
                ...
                // 인기도
                showpopularity = a['showpopularity']
                ...
                temp =
                    ` <div class = "card">
                        <button id="lovebtn" onclick="lovefunc(${id}),load()" type="button">♥︎</button>
...
function anime() {
    ...
    movies.forEach((a) => {
        ...
        temp =
            ` <div class = "card">
            <button id="lovebtn" onclick="lovefunc(${id}),anime()" type="button">♥︎</button>
            <p class="love">${love} times loved this movie</p>  
            <div class="card-body" onclick = 'alert("영화 ID : ${id}")' > 
            <img src="${poster_path}"
            class="poster_path">
...
function mychart() {
    ...
    movies.forEach((a) => {
        ...
        temp =
            ` <div class = "card">
            <button id="lovebtn" onclick="lovefunc(${id}),mychart()" type="button">♥︎</button>
            <p class="love">${love} times loved this movie</p>  
            <div class="card-body" onclick = 'alert("영화 ID : ${id}")' > 
            <img src="${poster_path}"
            class="poster_path">
